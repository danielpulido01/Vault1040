...
export function AnnualReportPage() {
  ...
  // External payment confirmation state
  const [paymentConfirmed, setPaymentConfirmed] = useState(false);
  const [externalPaymentMethod, setExternalPaymentMethod] = useState<string | null>(null);
  ...
  const fetchPrefillData = async (token: string) => {
    try {
      ...
      const { client, sunbizData, tokenId, paymentConfirmed: isPaymentConfirmed, externalPaymentMethod: extPaymentMethod } = response.data.data;
      ...
      // Set external payment confirmation status
      if (isPaymentConfirmed) {
        setPaymentConfirmed(true);
        setExternalPaymentMethod(extPaymentMethod || null);
      }
    } catch (error: unknown) {
